...
module battery_box() {
    ...
    difference() {
        ...
        union() {
            ...
            translate([(-B_HOLE_W - BATTERY_W)/2 + 0.01, -0.01 + (B_HOLE_L/2) + BACK_BOX_L_OFF - WALL_THICK, B_HOLE_H/2]) cube([B_HOLE_W, B_HOLE_L, B_HOLE_H*2], center=true);
            translate([0, BATTERY_LEDS_C_OFF, 0]) cube([BATTERY_LEDS_W, BATTERY_LEDS_L, 20], center=true);

            translate([0, 56, 0]) cube([50, 55, 40], center=true);
        }
    }
}

MOTOR_BOX_HOLE_LEVEL = 4;
MOTOR_BOX_HOLE_H = 20;
MOTOR_BOX_FRONT_HOLE_W = BATTERY_W;
MOTOR_BOX_FRONT_HOLE_L = 35;
MOTOR_BOX_BACK_HOLE_W = 32;
MOTOR_BOX_BACK_HOLE_L = 60;

module motor_box_bottom() {
    H = (MOTOR_H / 2) + FLOOR_THICK;

    BOTTOM_HOLE_W = MOTOR_BOX_BACK_HOLE_W - 6;
    BOTTOM_HOLE_L = MOTOR_BOX_BACK_HOLE_L - 6;
